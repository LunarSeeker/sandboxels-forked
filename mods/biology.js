// Original biology.js by Nekonico
...
renderPresets.FLESHBURN = function (pixel, ctx) {
    ...
    var ratio = pixel.char / 20
    if (ratio < 0.5) { return }
    if (ratio > 1) { ratio = 1 }
    drawSquare(ctx, "rgb(0,0,0)", pixel.x, pixel.y, 1, Math.min(0.8, (ratio - 0.5) * 0.8))
}
...
elements.blood_vessel = {
    ...
    reactions: {
        ...
        "mercury": {
            elem2: null, elem1: "infected_vessel", func: function (pixel1, pixel2) {
                if (pixel1.poisoned != true) {
                    pixel1.poisoned = true
                }
            }, chance: 0.005
        },
        "vaccine": {
            elem2: null, func: function (pixel1, pixel2) {
                if (pixel1.immune != true) {
                    pixel1.immune = true
                }
            }, chance: 0.01
        },
...
elements.appendix = {
    ...
    tick: function (pixel) {
        ...
        if (!isEmpty(pixel.x - 1, pixel.y, true)) {
            ...
            else if (elements[hitPixel.element].id === elements.gaseous_material.id && Math.random() > 0.95) {
                changePixel(hitPixel, "stench")
                pixel.nutrition += hitPixel.nutrition
                hitPixel.nutrition = 0
                if (hitPixel.poisoned === true && Math.random() > 0.9) {
