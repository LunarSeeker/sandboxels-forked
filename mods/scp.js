...
elements.body.reactions.level_0 = {
    chance: 0.3, func: function (pixel1, pixel2) {
...
elements.black_acid = {
    ...
    ignore: ["head_096", "body_096", "scp_055", "head", "body", "body_008", "head_008", "scp_035", "body_035", "head_035", "glass_shard", "porcelain_shard", "rad_shard", "color_sand", "sand", "iron", "steel", "glass", "rad_glass", "stained_glass", "acid_gas", "neutral_acid", "acid_cloud", "water", "salt_water", "sugar_water", "dirty_water", "copper", "gold", "porcelain", "plastic", "bead", "microplastic", "molten_plastic", "pool_water", "chlorine", "hydrogen", "oxygen", "ozone", "gold_coin", "silver", "nickel", "calcium"],
    reactions: {
...
elements.scp_049 = {
    ...
    onPlace: function (pixel) {
        ...
        else if (!isEmpty(pixel.x, pixel.y + 1, true) && pixelMap[pixel.x][pixel.y + 1].element === "body_049") {
            deletePixel(pixel.x, pixel.y + 1)
        }
        ...
        if (isEmpty(pixel.x, pixel.y - 1)) {
            ...
            while (movesToTry.length > 0) {
                ...
                if (isEmpty(pixel.x + move[0], pixel.y + move[1] - 1)) {
                    ...
                    var hitPixel = pixelMap[pixel.x + move[0]][pixel.y + move[1]]
                    if (hitPixel.element === "body_049" || hitPixel.element === "head_049" && hitPixel.panic < pixel.panic) {
                        // interact with other scp_049
...
elements.scp_055 = {
    behavior: [
        "XX|XX|XX",
        "XX|XX|XX",
        "M2|M1 AND EX:60>redacted,redacted,redacted,redacted,redacted,redacted|M2"
    ],
    category: "scp",
    color: "#000000",
    excludeRandom: true,
...
elements.scp_140 = {
    ...
    tick: function (pixel) {
        if (pixel.lines < 1 && isEmpty(pixel.x, pixel.y - 1)) {
            releaseElement(pixel, "daevite", 1, true)
            pixel.lines = 25
        }
        if (Math.random() < 0.5) {
            shuffleArray(squareCoordsShuffle)
            for (var i = 0; i < squareCoordsShuffle.length; i++) {
                var x = pixel.x + squareCoordsShuffle[i][0]
                var y = pixel.y + squareCoordsShuffle[i][1]
                if (!isEmpty(x, y, true) && (pixelMap[x][y].element === "urine" || pixelMap[x][y].element == "blood")) {
                    deletePixel(x, y)
                    pixel.lines -= 1
                    break
                }
...
elements.daevite = {
    ...
    onPlace: function (pixel) {
        ...
        else if (!isEmpty(pixel.x, pixel.y + 1, true) && pixelMap[pixel.x][pixel.y + 1].element === "body_daevite") {
            deletePixel(pixel.x, pixel.y + 1)
        }

        if (isEmpty(pixel.x, pixel.y + 1)) {
            createPixel("body_daevite", pixel.x, pixel.y + 1)
            var color = pixel.color
            changePixel(pixel, "head_daevite")
            pixel.color = color
        }
        else if (isEmpty(pixel.x, pixel.y - 1)) {
            createPixel("head_daevite", pixel.x, pixel.y - 1)
            pixelMap[pixel.x][pixel.y - 1].color = pixel.color
            changePixel(pixel, "body_daevite")
        }
        else {
            deletePixel(pixel.x, pixel.y)
        }
    },
    related: ["body_daevite", "head_daevite"],
    cooldown: defaultCooldown,
    hidden: true,
    forceSaveColor: true
}

elements.body_daevite = {
    color: ["#789406", "#999904", "#a8b05f"],
    category: "scp",
    hidden: true,
    density: 1500,
    state: "solid",
...
elements.head_daevite = {
    ...
    reactions: {
        ...
        "fallout": { elem1: ["ash", "meat", "rotten_meat", "cooked_meat"], chance: 0.03 },
        "plague": { elem1: "plague", chance: 0.05 },
        "oxygen": { elem2: "carbon_dioxide", chance: 0.6 },
        "beans": { elem2: [null, null, null, null, null, null, null, null, "stench"], chance: 0.2 },
        "sun": { elem1: "cooked_meat" },
        "light": { stain1: "#825043" },
        "bee": { stain1: "#cc564b", chance: 0.2 },
    ...
    tick: function (pixel) {
        ...
        // Find the body
        if (!isEmpty(pixel.x, pixel.y + 1, true) && pixelMap[pixel.x][pixel.y + 1].element == "body_daevite") {
            var body = pixelMap[pixel.x][pixel.y + 1]
            if (body.dead) { // If body is dead, kill head
                pixel.dead = body.dead
            }
...
elements.body_daevite.reactions = elements.body.reactions
elements.head_daevite.reactions = elements.head.reactions
...
elements.scp_229 = {
    ...
    colorPattern: [
        ...
        "govvvvwvvvbvvvyvvvVVrVVVVvvvvvvvcrvvvvvvvvvvvtvo",
    ],
    colorKey: {
        "C": "#a95232",
        "c": "#be4322",
...
elements.scp_409 = {
    ...
    onCollide: function (_pixelOG, pixel) {
        ...
        if (elements[pixel.element].state !== "liquid" && elements[pixel.element].state !== "gas" && Math.random() > 0.1) {
            if (elements[pixel.element].category !== "life" && elements[pixel.element].category !== "food" && pixel.element !== "wood" && pixel.element !== "bamboo" && pixel.element !== "skin") {
                changePixel(pixel, "scp_409")
            }
            else if (Math.random() > 0.75) {
                changePixel(pixel, "scp_409")
                if (Math.random() > 0.97) {
                    explodeAt(pixel.x, pixel.y, 10, ["quartz_crystal", "quartz_crystal", "quartz_crystal", "quartz_crystal", null, null, null, null])
                }
            }
        }
    },
...
elements.scp_447 = {
    ...
    reactions: {
        "ash": { elem2: null },
        "black_acid": { elem1: "poison_gas", elem2: "poison_gas" },
        "body_008": { elem2: "ash", func: function (_pixel1, pixel2) { explodeAt(pixel2.x, pixel2.y, 100, ["radiation", "fire", "plasma", "poison_gas", "acid_gas"]) } },
        ...
        "rotten_meat": { elem2: "ash", func: function (_pixel1, pixel2) { explodeAt(pixel2.x, pixel2.y, 100, ["radiation", "fire", "plasma", "poison_gas", "acid_gas"]) } },
...
elements.scp_882 = {
    ...
    colorPattern: [
        ...
        "BBBBBBBBBBBBBBBBAAAAAAAABBBBBBBBBBBBBBBB",
